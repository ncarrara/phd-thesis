...
\begin{document}
    ...
    \begin{frame}{Résoudre un MDP}
        \begin{itemize}
            \item Politique $\policy\in\cM(\cA)^{\cS}$ % that maps states to actions
            \item Retour $\return^{\policy} = \sum_{\indextransition=0}^\infty \discountfactor^{\indextransition} \reward(s_{\indextransition},a_{\indextransition})$% expliquer s_i et a_i
    ...
    \begin{frame}{Pourquoi transférer ?}
        ...
        \begin{figure}
            \begin{center}
                ...
                \caption{Les objectifs de l'apprentissage par transfert~\parencite{langley2006,LazaricSurvey}}
    ...
    \begin{frame}{Les différentes formes de transfert}
        ...
        \begin{table}
            ...
            \begin{tabular}{llll}
                \hline
                & Connaissance & Cadre & Performance \\ \hline

                \hline
                \textbf{cross domain} &&&\\
                \hline
                \cite{Gasic2013}& paramètre (transitions/politique)  & \makecell[l]{mono} & tous         \\
                \cite{Keizer2018-tl-dialogue}& paramètre (politique)& \makecell[l]{generique} & tous      \\
                \cite{Chen2018-tl-dialogue}& paramètre (politique) & \makecell[l]{generique} & tous       \\
                \cite{Ilievski2018-tl-dialogue}& paramètre (politique) & \makecell[l]{mono}& tous      \\
                \hline
                \textbf{user adaptation} &&&\\
                \hline
                \cite{casanueva2015-tl-dialogue}& paramètre (transitions/politique) & \makecell[l]{multi} & tous         \\
                \cite{Genevay2016}& \makecell[l]{transitions \\ paramètre (politique)} & \makecell[l]{multi}& tous   \\
                \cite{Mo2018-tl-dialogue-PETAL}& paramètre ($\Q$-fonction, DST) & \makecell[l]{generique}& tous        \\
                \hline
                \textbf{dialogue evaluation} &&&\\
                \hline
                \cite{elasri2014-tl-dialogue}& representation (reward fonction) & \makecell[l]{generique} & tous        \\
                \hline
    ...
    %            \begin{itemize}
    %                \item \cplus Dialogues très légérement plus sûr. % on garde donc l'user
    %                \item \cmoins mais cela n'affecte pas le score
    ...
    \begin{frame}{Cadre}
        ...
        \begin{block}{}
            \begin{itemize}
                \item \cplus \textit{Tractable}
                \item \cplus DOF supplémentaire % A L ORAL On peut définir un budget de safety , formuation plus naturelle
    ...
    \begin{frame}{Equation d'optimalité de Bellman}
        \begin{theorem}[Equation d'optimalité de Bellman]
            $Q^*$ vérifie:
            \begin{align*}
                Q^{*}(\os, \oa) &= \cT Q^{*}(\os, \oa)\\
