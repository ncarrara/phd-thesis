...
\begin{document}
    ...
    \begin{frame}{Une formulation plus directe}
        ...
        \begin{itemize}
            ...
            \item Formulation lagragienne insuffisante
    ...
    \begin{frame}{Setting}
        ...
        \begin{block}{}
            \begin{itemize}
                ...
                \item \colorbox{red}{(-) Calibration nécessaire}
    ...
    \begin{frame}{Setting}
        ...
        \begin{block}{}
            \begin{itemize}
                \item \colorbox{green}{(+) \textit{Tractable}}
                \item \colorbox{green}{(+) DOF supplémentaire}% A L ORAL On peut définir un budget de safety et d'afranchir des lambda.}
                \item \colorbox{red}{(-) budget fixé}
    ...
    \begin{frame}{Setting}
        ...
        \begin{block}{}
            \begin{itemize}
                ...
                \item \colorbox{green}{(+) Le budget est un paramètre du modèle} % A L ORAL (calcul le front de pareto)}
                \item \colorbox{red}{(-) \textit{Intractable}}
            \end{itemize}
        \end{block}
    ...
    \begin{frame}{Comportement asymptotique}
        ...
        \begin{itemize}
            \item  Si $|\cA| \geq 2$, \colorbox{red}{$\cT$ n'est pas une contraction}
    ...
    \begin{frame}{Budgeted Fitted-Q}
        ...
        \begin{itemize}
            \item $\cD=\{(\os_i,\oa_i,r_i,\os_i'\}_{i\in [0,N]}$
            \item $\hat{\cT} Q(\os, \oa, r, \os') \eqdef r + \gamma \sum_{\ov{a'}\in \cA} \pi_\text{greedy}(\ov{a'}|\ov{s'}; Q) Q(\ov{s'}, \ov{a'}).$
            \pause \item $\cL(Q_\theta, Q_\text{target};\cD) = \sum_{\cD} ||Q_\theta(\os, \oa) - Q_\text{target}(\os, \oa, r, \os')||_2^2$
        \end{itemize}
    \end{frame}
