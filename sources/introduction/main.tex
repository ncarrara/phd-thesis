...
\Cref{part:contributions-scaling} lists the several contributions of the thesis to scale \glspl{DS} to a growing base of users: \Cref{chapter:sigdial} introduces a novel method for scaling \gls{TL} in \idx{user adaptation} applications.This method extends an existing framework with \idx{clustering} of \glspl{DS}; \Cref{sec:continuous} designs a \gls{TL} solution for \gls{DQN}.
...
Finally, the conclusion in \Cref{chapter:conclusion}, followed by the the appendixes in \Cref{chap:appendix}, brings down the curtain on this thesis.