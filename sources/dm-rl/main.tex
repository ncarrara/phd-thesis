...
\end{figure}
%
The state of an \gls{MDP} must contain all past observations. In other words, the \idx{agent} would not be able to take a better decision with additional past information to complete the dialogue task. We say that the process is stateless and respects the Markov property (or that it is \textit{Markovian}).
\begin{definition}
